...
describe('InlineAddRow', () => {
  ...
  it('starts with plus button and Add item field only', () => {
    render(<InlineAddRow onSave={mockOnSave} campMealOptions={campMealOptions} />);

    expect(
      screen.getByRole('button', { name: /show store, meal, and quantity/i }),
    ).toBeInTheDocument();
  ...
  it('reveals store, meal, and quantity when plus is clicked with empty name', async () => {
  ...
  it('can save after expanding via plus and filling the form', async () => {
    const user = userEvent.setup();
    render(<InlineAddRow onSave={mockOnSave} campMealOptions={campMealOptions} />);
    ...
    await user.type(screen.getByLabelText('Add item'), 'Tomatoes');
    await user.selectOptions(screen.getByLabelText('Store lane'), 'Aldi');
    await user.selectOptions(screen.getByLabelText('Menu meal'), 's2');
